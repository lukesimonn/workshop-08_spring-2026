...
# creating clean data frame object
drought_exp_clean <- drought_exp |> 
  # standardizing column names (removing spaces and .)
  clean_names() |> 
  # changing species codes to full names
  ...
  # changing treatment codes to full names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # reorganizing data by species and treatment type
...
slice_sample(drought_exp_clean, # display clean data frame
             n = 10) # show 10 rows
```
...
ggplot(data = drought_exp_clean,
       aes(x = sla, # x-axis specific leaf area
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Treatment type",
       ...
       title = "Well watered individuals tend to have higher mass")
...
ggplot(data = drought_exp_clean,
       ...
       # y axis: mass
       # color by species
       aes(x = reorder(species_name, -total_g, FUN = mean),
           y = total_g,
           color = species_name)) +
  # jitter to represent indivs in each species
  geom_jitter(height = 0,
              width = .2) +
  ...
  # coloring by species with custom colors
  scale_color_manual(
    values = c(
...
- residuals are normally distributed (QQ)
...
72.6% of the variation of plant mass is explained by this model.
...
- `species_nameEschscholzia californica`: difference between Escholzia californica mass in the well watered treatment and Lotus scoparius mass in the well watered treatment
...
preds <- ggpredict(
  ...
  # renaming columns to fit original dataframe
